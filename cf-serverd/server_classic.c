...
RSA *newkey = RSA_new();
{
    ...
    int len_n = ReceiveTransaction(conn->conn_info, recvbuffer, NULL, -1);
    if (len_n == -1)
    {
        Log(LOG_LEVEL_ERR, "Authentication failure: "
...
{
    int len_e = ReceiveTransaction(conn->conn_info, recvbuffer, NULL, -1);
    if (len_e == -1)
    {
        Log(LOG_LEVEL_ERR, "Authentication failure: "
...
/* proposition C4 - Receive counter-challenge response. */
{
    ...
    int recv_len = ReceiveTransaction(conn->conn_info, recv_buf, NULL, -1);
...
/* proposition C5 - Receive session key */
{
    ...
    int keylen = ReceiveTransaction(conn->conn_info, session_key, NULL, -1);
