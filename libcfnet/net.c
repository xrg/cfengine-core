                    ...
                    const char *buffer, int len, char status)
{
    ...
    case CF_PROTOCOL_CLASSIC:
        ret = SendSocketStream(ConnectionInfoSocket(conn_info), work,
                               wlen + CF_INBAND_OFFSET);
        if ((ret != -1) && (wlen == 0) && (len))
        {
            ret = SendSocketStream(ConnectionInfoSocket(conn_info), buffer, len);
        }
        break;
    case CF_PROTOCOL_TLS:
        ret = TLSSend(ConnectionInfoSSL(conn_info), work, wlen + CF_INBAND_OFFSET);
        if ((ret != -1) && (wlen == 0) && (len))
        {
            ret = TLSSend(ConnectionInfoSSL(conn_info), buffer, len);
        }
        break;
    default:
 ...
 */
int ReceiveTransaction(ConnectionInfo *conn_info, char *buffer, int *more, int bufsize)
{
