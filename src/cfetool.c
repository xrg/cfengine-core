...
int Check(double value, time_t u_time, int dbtype, int *bucket)
{
  ...
  *bucket = CF_GRAINS / 2 + 
            (int)(0.5 + (VALUE - average.expect) * 
                  CF_GRAINS / (4.0 * sqrt(average.var)));
...
struct Average EvalAvQ(char *t, int dbtype, int update)
{
  ...
  newvals.var =
    WAverage((new_value - newvals.expect) * (new_value - newvals.expect),
             currentvals->var, WAGE);
...
if (new - average > 0)
  ...
  delta =
    sqrt((new - average) * (new - average) +
         (new - localav) * (new - localav));
...
#else
  if ((errno = dbp->open(dbp, NULL, AVDB, NULL, DB_BTREE, flags, 0644))
      != 0)
#endif
  {
